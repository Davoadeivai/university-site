...
# نگارشِ درستِ نام‌هایی که با فاصلهٔ کامل نوشته شده بودند.
RENAMES = {
    'گروه معماری و نقشه کشی': 'گروه معماری و نقشه‌کشی',
    'گروه روانشناسی': 'گروه روان‌شناسی',
}
...
DEFAULT_GROUPS = [
    ...
    },
    {
        ...
        'name': 'گروه روان‌شناسی',
        'description': 'گروه روان‌شناسی با تدریس مبانی روانشناسی عمومی، بالینی، صنعتی-سازمانی و مشاوره، دانشجویان را برای فعالیت در حوزه بهداشت روان آماده می‌کند.',
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # نام قدیمی → نام تازه.
        #
        # بدون این، تغییرِ نگارشِ یک نام یعنی ساختن گروهِ تازه کنار
        # گروهِ قدیمی: دو «معماری و نقشه‌کشی» با رشته‌های تقسیم‌شده
        # میانشان. اول نام‌ها اصلاح می‌شوند، بعد فهرست خوانده می‌شود.
        for old_name, new_name in RENAMES.items():
            moved = AcademicGroup.objects.filter(name=old_name).exclude(
                name=new_name).update(name=new_name)
            if moved:
                self.out('  ~ %s → %s' % (old_name, new_name))
