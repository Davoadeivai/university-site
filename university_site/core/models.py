...
# اسلایدهای صفحهٔ اصلی سقف ندارند.
#
# اول عدد ۵ داخل ویو نوشته شده بود، بعد سقف ۱۲ در همین‌جا؛ هر دو یک
# رفتار داشتند — مدیر اسلاید می‌ساخت، در پنل ثبت می‌شد و روی صفحه
# نمی‌آمد، بی‌آنکه جایی بگوید چرا.
#
# نگرانیِ وزنِ صفحه با سقف حل نمی‌شد، با بارگذاری تنبل حل می‌شود:
# جز اسلاید اول، بقیه تا وقتی دیده نشوند دانلود نمی‌شوند. پس تعداد
# را خودِ موسسه تعیین می‌کند و «همه» هم یک انتخاب است.
#
# این عدد فقط پیش‌فرضِ کادر پنل مانده، نه سقف.
...
class SiteSettings(ShrinkImagesMixin, models.Model):
    ...
    ticker_seconds = models.PositiveSmallIntegerField(
        # برچسب باید جهت را خودش بگوید: عدد ۱ گذاشتند و انتظار
        # داشتند آهسته شود.
        _('سرعت نوار فوری — ثانیه برای یک عبور (بزرگ‌تر = آهسته‌تر)'),
        default=60,
        # کفِ ۵ ثانیه حدسِ ما بود، نه خواستهٔ موسسه، و سرِ راهِ تنظیم
        # سرعت ایستاد. یک ثانیه تند است ولی انتخابِ خودشان است.
        validators=[MinValueValidator(1), MaxValueValidator(300)],
        help_text=_('هر اطلاعیه چند ثانیه طول بکشد تا از لبهٔ چپ صفحه تا '
                    'لبهٔ راست برود. عدد بزرگ‌تر یعنی آرام‌تر — ۴۵ تا ۶۰ '
                    'برای خواندن راحت است و زیر ۱۰ عملاً خوانده نمی‌شود.'))
    ticker_hold_seconds = models.PositiveSmallIntegerField(
        # پیش‌فرض صفر: نوارِ بی‌وقفه همان چیزی است که موسسه می‌خواست،
        # و مکث انتخابی است، نه چیزی که باید خنثایش کرد.
        _('مکث پیش از ورود خبر (ثانیه)'), default=0,
        validators=[MinValueValidator(0), MaxValueValidator(60)],
        help_text=_('نوار این چند ثانیه خالی می‌ماند و بعد خبر از سمت چپ '
                    'وارد می‌شود. این زمان به «زمان هر خبر» اضافه '
                    'می‌شود، از آن کم نمی‌کند. صفر یعنی بی‌وقفه.'))
    ...
    home_slider_count = models.PositiveSmallIntegerField(
        _('تعداد اسلاید صفحهٔ اصلی'), default=DEFAULT_HOME_SLIDES,
        help_text=_(
            'چند اسلاید از فهرست اسلایدها روی صفحهٔ اصلی بیاید. '
            '<strong>صفر یعنی همه</strong> — هر اسلاید فعالی که ساخته‌اید. '
            'سقفی در کار نیست: جز اسلاید اول، بقیه تا وقتی دیده نشوند '
            'دانلود نمی‌شوند.'))
