...
// ============================================================
//  صفحهٔ ریاست — سه فهرست سابقه به زبانه تبدیل می‌شوند
// ============================================================
// سوابق تحصیلی، اجرایی و پژوهشی پشت سر هم، صفحه را چند برابر بلند
// می‌کردند در حالی که خواننده معمولاً یکی‌شان را می‌خواهد.
//
// عمداً بهبود تدریجی است: قالب هر سه بخش را باز رندر می‌کند و اینجا
// جمع می‌شوند. اگر این فایل نرسد یا خطا بخورد، صفحه همان چیزی است
// که قبلاً بود — نه یک بخش خالی.
(function () {
    'use strict';

    var host = document.querySelector('[data-pres-tabs]');
    if (!host) return;

    var panels = Array.prototype.slice.call(
        host.querySelectorAll('.pres-panel'));
    // یک بخش که زبانه ندارد؛ دو تا به بالا ارزشش را دارد
    if (panels.length < 2) return;

    var list = document.createElement('div');
    list.className = 'pres-tablist';
    ...
    panels.forEach(function (panel, i) {
        ...
        button.type = 'button';
        button.className = 'pres-tab';
        button.setAttribute('role', 'tab');
        button.id = 'pres-tab-' + i;
        ...
        button.appendChild(document.createTextNode(
            ' ' + (panel.getAttribute('data-tab-title') || 'بخش ' + (i + 1))));
        ...
        // عنوان داخل پنل با متن زبانه یکی است؛ دو بار خواندنش اضافی است
        var heading = panel.querySelector('.pres-h2');
        if (heading) heading.hidden = true;
    });

    // کلیدهای جهت‌دار — انتظار استاندارد از یک tablist. در RTL،
    // فلش راست یعنی «قبلی»، پس جهت برعکسِ لاتین است.
    list.addEventListener('keydown', function (event) {
        ...
        else if (event.key === 'Home') { select(0, true); event.preventDefault(); return; }
        else if (event.key === 'End') { select(buttons.length - 1, true); event.preventDefault(); return; }
        if (!step) return;
    ...
    // نوار پرش بالای صفحه به همین بخش‌ها لینک می‌دهد. بدون این،
    // کلیک روی «سوابق اجرایی» به یک پنل پنهان می‌رفت و هیچ اتفاقی
    // نمی‌افتاد.
    function openFor(hash) {
        var id = (hash || '').replace('#', '');
        if (!id) return false;
        for (var i = 0; i < panels.length; i++) {
            if (panels[i].id === id) { select(i, false); return true; }
        }
        return false;
    }
    ...
    document.addEventListener('click', function (event) {
        ...
        if (!link) return;
        if (openFor(link.getAttribute('href'))) {
            // پنل تازه باز شده؛ مرورگر باید بعد از نمایش، جایش را
            // پیدا کند
