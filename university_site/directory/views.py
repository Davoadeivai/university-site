...
# هیئت مؤسس و هیئت امنا عمداً اینجا نیستند.
#
# برای آن دو رکن، صفحه و مدل جداگانه‌ای وجود دارد («عضو هیات») که
# بیوگرافی و تحصیلات و تخصص هم نگه می‌دارد. تا امروز هر دو دستگاه
# روشن بودند: پنل «عضو هیات» را ویرایش می‌کرد و منو صفحهٔ ساخته‌شده
# از «افراد موسسه» را نشان می‌داد — یعنی هر ویرایشی بی‌اثر به‌نظر
# می‌رسید و هیچ‌جا نمی‌گفت چرا.
#
# حالا منو به همان صفحه‌ای می‌رود که پنل ویرایشش می‌کند، و این فهرست
# فقط سه بخشِ باقی‌مانده را می‌سازد.
RETIRED_SECTIONS = {
    'هیات-موسس': 'core:board_founders',
...
def academic_people_section(request, slug):
    ...
    # نشانی‌های قدیمیِ دو رکن هنوز در بوکمارک‌ها و موتورهای جست‌وجو
    # هستند؛ به صفحهٔ تازه می‌روند، نه به ۴۰۴.
    if slug in RETIRED_SECTIONS:
        from django.shortcuts import redirect
        return redirect(RETIRED_SECTIONS[slug], permanent=True)
