...
CONTENT_COMMANDS = [
    # افراد موسسه، پیش از هر چیز دیگری.
    #
    # این فهرست ۱۰۳ نفر است — هیئت مؤسس، هیئت امنا، هیئت علمی، مدیران
    # گروه، مدرسین و کارکنان — و صفحه‌های «ارکان موسسه» و «هیئت علمی»
    # از آن پر می‌شوند. تا امروز روی دیپلوی اجرا نمی‌شد، پس هر پایگاه
    # داده‌ای که این دستور روی آن دستی اجرا نشده بود، فهرستی ناقص یا
    # کهنه نشان می‌داد.
    #
    # بی‌خطر است: بدون فلگ فقط جاهای خالی را پر می‌کند و نامی را که
    # موسسه در پنل اصلاح کرده بازنویسی نمی‌کند. تنها حذفش، ادغام
    # ردیف‌های تکراری از یک نفر است.
    #
    # پیش از set_group_heads می‌آید، چون آن دستور مدیران گروه را از
    # همین فهرست می‌خواند.
    'seed_directory',           # ۱۰۳ نفر سند «افراد موسسه»

    # واحدهای حوزهٔ ریاست — و دو دبیرخانه‌ای که زیر «ارکان موسسه»
    # نشسته‌اند.
    #
    # منو به این صفحه‌ها لینک می‌دهد ولی ردیفشان را همین دستور
    # می‌سازد، و تا امروز روی دیپلوی اجرا نمی‌شد: «دبیرخانه هیأت
    # مؤسس» و «دبیرخانه هیأت امناء» روی سرور ۴۰۴ می‌دادند.
    #
    # بی‌خطر: بی‌فلگ فقط فیلدهای خالی را پر می‌کند، و واحدی را که
    # موسسه در پنل خاموش کرده دوباره روشن نمی‌کند.
    'seed_presidency',          # واحدهای حوزهٔ ریاست و دبیرخانه‌ها
    'seed_groups',              # یازده گروه آموزشی، و اصلاح نگارش نامشان
    ...
    # کاشی «ارسال فیش واریزی» در دسترسی سریع صفحهٔ اصلی.
    #
    # فقط اگر نبود می‌سازد و عنوانی را که موسسه در پنل عوض کرده
    # بازنویسی نمی‌کند — برخلاف sync_official_eservices که پیش از
    # کاشتن همه را پاک می‌کند و برای همین در این فهرست نیست.
    'seed_receipt_link',
]
