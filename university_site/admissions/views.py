...
def apply(request):
    ...
    if request.method == 'POST':
        ...
        if dup:
            # کد رهگیری نباید اینجا چاپ شود: کد ملی محرمانه نیست و کد رهگیری
            # به‌تنهایی پرونده را باز می‌کند؛ یعنی هر کس کد ملی دیگری را
            # می‌دانست، با همین پیام به پرونده‌اش می‌رسید.
            errors.append(
                'قبلاً درخواست فعالی با این کد ملی یا شماره موبایل ثبت شده است. '
                'برای دیدن وضعیت از «پیگیری وضعیت» با کد ملی (و تأیید پیامکی) اقدام کنید. '
